...
def load_selected_questions(dev_path, selected_path):
    # 加载完整dev数据
    ...
    for s in selected:
        ...
        for entry in dev_data:
            if (entry['question'] == s['question'] and 
                entry['db_id'] == s['db_id']):
                gold_entries.append(f"{entry['query']}\t{entry['db_id']}")
                pred_entries.append(f"{s['pred_sql']}\t{entry['db_id']}")
                found = True
                break
        if not found:
            raise ValueError(f"问题未找到: {s['question']}")
    
    # 创建临时文件
    with open("tmp_gold.txt", "w") as f:
        f.write("\n".join(gold_entries))
    with open("tmp_pred.txt", "w") as f:
        f.write("\n".join(pred_entries))
    
    return "tmp_gold.txt", "tmp_pred.txt"

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--dev', type=str, help='原始dev.json路径')
    parser.add_argument('--selected', type=str, help='selected_questions.json路径')
    ...
    # 生成临时评估文件
    gold_tmp, pred_tmp = load_selected_questions(args.dev, args.selected)
    
    kmaps = build_foreign_key_map_from_json(args.table)
    evaluate(gold_tmp, pred_tmp, args.db, args.etype, kmaps)
